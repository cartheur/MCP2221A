...
using static Smdn.Devices.MCP2221.MCP2221;
...
#pragma warning restore IDE0040
  ...
  public sealed class GP1Functionality : GPFunctionality, IInterruptDetectionFunctionality, IADCFunctionality, IClockOutputFunctionality {
    ...
    public void RetrieveADCData(CancellationToken cancellationToken = default)
      => GetADCData(
        pinDesignation: "ADC1",
        gpDesignation: GPDesignation.AlternateFunction1,
        cancellationToken: cancellationToken
  ...
  public sealed class GP2Functionality : GPFunctionality, IADCFunctionality, IDACFunctionality {
    ...
    internal GP2Functionality(MCP2221 device)
      : base(device) { }
    ...
    public void RetrieveADCData(CancellationToken cancellationToken = default) => throw new NotImplementedException();
  }
  ...
  public abstract partial class GPFunctionality {
    ...
    private static class GetADCDataCommand
    {
      public static void ConstructCommand(Span<byte> comm, ReadOnlySpan<byte> userData, Memory<byte> adcData)
#pragma warning restore IDE0060
      {
        // [MCP2221A] 3.1.14 GET ADC VALUE
        comm[0] = 0x61; // Get ADC Value by SRAM dump
      }

      public static bool ParseResponse(ReadOnlySpan<byte> resp, Memory<byte> adcData)
      {
        resp.Slice(22, 64).CopyTo(adcData.Span); // ADC1-3 Settings
...
#pragma warning restore IDE0060, SA1313
    ...
    private protected void GetADCData(
      string pinDesignation,
      GPDesignation gpDesignation,
      CancellationToken cancellationToken = default
      )
    {
      var adcData = ArrayPool<byte>.Shared.Rent(4);

      try {
        // retrieve current ADC1-ADC3 settings
        device.Command(
          userData: default,
          arg: adcData.AsMemory(0, 4),
          cancellationToken: cancellationToken,
          constructCommand: GetGPSettingsCommand.ConstructCommand,
          parseResponse: GetGPSettingsCommand.ParseResponse
        );
        // var resp = this._hid_xfer(new byte[] { 0x10 });      <-- from python
        // return resp[49 + 2 * pin] << 8 | resp[48 + 2 * pin]; <-- from python
        // Read the data from the ADC pin (64-byte response report)
        // private const int CommandLength = 64; private const int ResponseLength = 64;
        //device.Command(
        //  userData: default,
        //  arg: ((ReadOnlyMemory<byte>)adcData.AsMemory(0, 4), GPIndex, gpDesignation),
        //  cancellationToken: cancellationToken,
        //  constructCommand: GetADCDataCommand.ConstructCommand,
        //  parseResponse: GetADCDataCommand.ParseResponse
        //);

        PinDesignation = pinDesignation;
      }
      finally {
        ArrayPool<byte>.Shared.Return(adcData);
      }
    }
